...
**All scripts accept EITHER a directory OR a single file for `--input`:**
- `--input /data` → recursively process every CSV/JSON in the mounted dataset
- `--input /data/sample.csv` → process just that one file
- `--input /data/subfolder` → recursive scan of a subfolder
