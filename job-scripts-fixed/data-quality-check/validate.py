...
def _read_one(f):
    """Read a single CSV/JSON file into a list of dicts. Unknown types skipped."""
    rows = []
    ext = f.suffix.lower()
    if ext == ".csv":
...
def load_data(input_path):
    """Load data — accepts a directory (batch) or a single .csv/.json file."""
    p = Path(input_path)
    if p.is_file():
        return _read_one(p)
    data = []
    for f in p.rglob("*.csv"):
        data.extend(_read_one(f))
...
def main():
    ...
    if not os.path.exists(args.input):
        print(f"[ERROR] Input path missing: {args.input}")
        sys.exit(1)
